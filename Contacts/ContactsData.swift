...
class DataManager{
    ...
    static let contacsData = DataManager()
    ...
    init() {
        ...
        for ((firstName, surName), (email, phoneNumber)) in
                zip(zip(firstNames, surNames), zip(emails, phoneNumbers)) {
            
            let person = Person(firstName: firstName, surName: surName,
